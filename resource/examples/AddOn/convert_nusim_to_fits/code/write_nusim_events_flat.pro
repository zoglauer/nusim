...
outpath = outpath +'/'+header_data.obsid+'_flat/event_uf'
print, outpath
...
FOR i = 0l, nfpma - 1 DO BEGIN
   ...
   ; Make it a flat field:
   thisx = round( (randomu(seed) * 60. + 40. ) )
   thisy = round( (randomu(seed) * 60. + 40. ) )
...
;   IF i MOD 1e4 EQ 0 THEN print, i, " of ", nfpmb
   ...
   ; Make it a flat field:
   thisx = round( (randomu(seed) * 60. + 20. ) )
   thisy = round( (randomu(seed) * 60. + 20. ) )
