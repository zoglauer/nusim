...
public:
  ...
  bool GenerateARF(TString FileName);
...
bool Target::OpenEvtFile(TString FileName)
{  
  //! Load and initialize the file
...
bool Target::WriteFitsSpectrum(TString FileName, TString Type, long data[], float exposure)
{
  ...
  if (Type == "source"){
    ...
    fits_write_key(m_File, TSTRING, "ANCRFILE",  const_cast<char*>("source.arf"), " ", &Status);  
    fits_write_key(m_File, TSTRING, "BACKFILE",  const_cast<char*>("bkg.fits"), " ", &Status);  
  }
...
bool Target::GenerateARF(TString FileName)
{

  OpenEvtFile(FileName);
  ...
  MVector region;
  region = LoadDSRegion("/Users/kristin/data/nusim/nusim/ds91oaa.reg");
  ...
  WriteFitsSpectrum("source.pha", "source", source, exposure);
  WriteFitsSpectrum("bkg.fits", "bkg", bkg, exposure);
  WriteARF("source.arf",Elow,Ehigh,ARF);
...
int main(int argc, char** argv)
{
  ...
  Target src;
  src.Initialize();
  src.GenerateARF("/Users/kristin/data/nusim/nusim/testcrab1oaa.events.fits");
