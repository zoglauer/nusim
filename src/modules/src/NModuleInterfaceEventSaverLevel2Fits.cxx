...
{  
  ...
  fits_create_file(&File, FileName, &Status);
  if (Status != 0) {
  ...
  fits_create_tbl(File, BINARY_TBL, nrow, tfield, 
  ...
  fits_write_key(File, TFLOAT, "TCDLT1", &tcdlt1, "Platescale", &Status); 	  
  fits_write_key(File, TFLOAT, "TCDLT2", &tcdlt2, "Platescale", &Status); 	  
  fits_write_key(File, TFLOAT, "TCRVL1", &tcrvl1, "Transform to celestrial coords", &Status); 	  
  fits_write_key(File, TFLOAT, "TCRVL2", &tcrvl2, "Transform to celestrial coords", &Status); 	  
  fits_write_key(File, TFLOAT, "TCRPX1", &tcrpx1, "Pixel reference point", &Status); 	  
  fits_write_key(File, TFLOAT, "TCRPX2", &tcrpx2, "Pixel reference point", &Status); 	  
  fits_write_key(File, TSTRING, "TCRUNI1", deg," ", &Status);
  fits_write_key(File, TSTRING, "TCRUNI2", deg," ", &Status);
  fits_write_key(File, TSTRING, "TCTYP1", tctyp1," ", &Status);
  fits_write_key(File, TSTRING, "TCTYP2", tctyp2," ", &Status); 
  fits_write_key(File, TSTRING, "RADESYS", radesys, " ", &Status);
 
   //! Write NuSim header keywords
  char version[10];
  strcpy(version,g_Version);
  
  fits_write_key(File, TSTRING, "NuSimVER", version, "NuSim version number", &Status);
  fits_write_key(File, TLONG, "NuSimSVN", &g_SVNRevision, "NuSim SVN reversion number", &Status);
...
bool NModuleInterfaceEventSaverLevel2Fits::SaveEventLevel2Fits(NEvent& Event)
{
  ...
  //! Save 1000 events at a time
    if (counter == 999) {
      ...
      fits_write_col(File, TFLOAT, 1, firstrow, firstelem, 1000, c1,
                   &Status);
      fits_write_col(File, TFLOAT, 2, firstrow, firstelem, 1000, c2,
                   &Status);
      fits_write_col(File, TFLOAT, 3, firstrow, firstelem, 1000, c3,
                   &Status);
...
bool NModuleInterfaceEventSaverLevel2Fits::CloseLevel2FitsFile()
{
  ...
  if (counter != 0) {
	...
	fits_write_col(File, TFLOAT, 1, firstrow, firstelem, counter, rc1,
                   &Status);
    fits_write_col(File, TFLOAT, 2, firstrow, firstelem, counter, rc2,
                   &Status);
    fits_write_col(File, TFLOAT, 3, firstrow, firstelem, counter, rc3,
                   &Status);
