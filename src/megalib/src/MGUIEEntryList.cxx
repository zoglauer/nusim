...
MGUIEEntryList::MGUIEEntryList(const TGWindow* Parent, TString Label, int Mode) :
  MGUIElement(Parent)
{
...
void MGUIEEntryList::Add(TString Label, int Value, bool Limits, int Min, int Max)
{
...
void MGUIEEntryList::Add(TString Label, double Value, bool Limits, double Min, double Max)
{
	MGUIEEntry* Entry = new MGUIEEntry(this, Label, false, Value, Limits, Min, Max);
  Entry->SetEntryFieldSize(m_Size);
...
void MGUIEEntryList::Add(TString Label, TString Value)
{
...
TString MGUIEEntryList::GetAsString(int Entry)
{
  //

  if (Entry < 0 || Entry >= GetNEntrys()) {
    Error("TString MGUIEEntryList::GetAsString(int Entry)",
          "Index out of bounds!");
    return "";
  }
...
void MGUIEEntryList::SetValue(int Entry, TString Value)
{
  if (Entry < 0 || Entry >= GetNEntrys()) {
    Error("MGUIEEntryList::SetValue(TString Value)",
          "Index out of bounds!");
    return;
  }
