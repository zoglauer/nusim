...
bool MFunction::Set(const TString FileName, const TString KeyWord, 
                    const unsigned int InterpolationType)
{
  ...
  for (unsigned int i = 0; i < Parser.GetNLines(); ++i) {
    ...
    if (Parser.GetTokenizerAt(i)->IsTokenAt(0, KeyWord) == true) {
    ...
    } else if (Parser.GetTokenizerAt(i)->IsTokenAt(0, "IP") == true && m_InterpolationType == c_InterpolationUnknown) {
      ...
      TString IP = Parser.GetTokenizerAt(i)->GetTokenAt(1);
      IP.ToLower();
      if (IP == "no" || IP == "none") {
...
bool MFunction::Save(const TString FileName, const TString Keyword)
{
  // Save the data:
...
double MFunction::GetRandom()
{
  ...
  if (m_Cumulative.size() == 0) {
    if (m_InterpolationType == c_InterpolationNone) {
      m_Cumulative.push_back(m_Y[0]);
      for (unsigned int i = 1; i < m_Y.size(); ++i) {
        m_Cumulative.push_back(m_Cumulative.back() + m_Y[i]);
      }
    } else {
