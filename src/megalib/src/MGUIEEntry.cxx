...
MGUIEEntry::MGUIEEntry(const TGWindow* Parent, TString Label, bool Emphasize, TString Value) :
...
MGUIEEntry::MGUIEEntry(const TGWindow* Parent, TString Label, bool Emphasize, 
...
void MGUIEEntry::SetValue(TString Value)
{
  m_Value = Value; 
...
TString MGUIEEntry::GetAsString()
{
...
TString MGUIEEntry::MakeSmartString(double Number)
{
  ...
  while (true) {
    if (TString(Str(Str.Length()-1)).CompareTo(".") == 0) { // if ... is true
      ...
      break;
    } else if (isdigit(Str(Str.Length()-1)) != 0) { // if ... is true
      if (TString(Str(Str.Length()-1)).CompareTo("0") == 0) {
        Str.Replace(Str.Length()-1, 1, TString(""));
      } else {
        break;
      }
...
bool MGUIEEntry::CheckForNumber(TString Number)
{
...
double MGUIEEntry::ToNumber(TString Number) 
{
