...
bool MFunction3DSpherical::Set(const TString FileName, const TString KeyWord,
    ...
    for (unsigned int y = 0; y < m_Y.size(); ++y) {
      for (unsigned int z = 0; z < m_Z.size(); ++z) {
        if (m_V[x + m_X.size()*y + m_X.size()*m_Y.size()*z] != 0) {
          out<<"AP "<<x<<" "<<y<<" "<<z<<" "<<m_V[x + m_X.size()*y + m_X.size()*m_Y.size()*z]<<endl;
        }
      }
    }
  }
...
void MFunction3DSpherical::GetRandom(double& x, double& y, double& z)
{
  ...
  // The follwoing is an accurate and safe version but rather slow...
