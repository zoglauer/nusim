  ...
  : TGTransientFrame(Root, Parent, w, h, options)
{
  ...
  TString Path(g_MEGAlibPath + "/resource/icons/global/Icon.xpm");
...
TString MGUIDialog::MakeSmartString(double Number)
{
  char c[20];
...
bool MGUIDialog::IsNumber(TString Str)
{
  Str.ReplaceAll("1", "");
  Str.ReplaceAll("2", "");
  Str.ReplaceAll("3", "");
...
double MGUIDialog::MakeNumber(TString Str)
{
  return atof((char *) Str.Data());
}
...
void MGUIDialog::AddSubTitle(TString SubTitle)
{
  // Add a label to the top of the window:
  ...
  if (m_SubTitleAdded == false) {
    ...
    while (SubTitle.Contains("\n") == true) {
      ...
      TString S = SubString.Remove(SubString.First("\n"), SubString.Length() - (SubString.First("\n")));
