...
bool MSystem::GetTime(long int& Seconds, long int& NanoSeconds)
{
  ...
  Seconds = time(0);
  NanoSeconds = 0;
  merr<<"No support for nanoseconds on windows!"<<endl;
#endif
...
bool MSystem::GetFileSuffix(TString Filename, TString* Suffix)
{
  ...
  *Suffix = TString(Filename.Replace(0, Filename.Last('.')+1, ""));
  return true;
}
...
bool MSystem::GetFileDirectory(TString Filename, TString* Directory)
{
  //  *Directory = TString(Filename.Replace(0, Filename.Last('/'), ""));
  *Directory = TString(gSystem->BaseName((char *) Filename.Data()));
...
bool MSystem::GetFileWithoutSuffix(TString Filename, TString* NewFilename)
{
  //*NewFilename = gSystem->DirName((char *) Filename.Data());
  *NewFilename = TString(Filename.Replace(Filename.Last('.'), Filename.Length(), ""));
  return true;
}
...
bool MSystem::FileExist(const TString Filename)
{
