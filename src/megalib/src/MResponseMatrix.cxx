...
const float MResponseMatrix::c_ShowX = numeric_limits<float>::max()/2;
const float MResponseMatrix::c_ShowY = numeric_limits<float>::max()/3;
const float MResponseMatrix::c_ShowZ = numeric_limits<float>::max()/4;
...
MResponseMatrix::MResponseMatrix(TString Name) : 
...
bool MResponseMatrix::Read(TString FileName)
{
  ...
  TString Type = Parser.GetFileType(); 
