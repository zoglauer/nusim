...
class MResponseMatrixO3 : public MResponseMatrix
{
 ...
 public:
  ...
  MResponseMatrixO3(vector<float> x1Dim, vector<float> x2Dim, 
                    vector<float> x3Dim);
  MResponseMatrixO3(TString Name, vector<float> x1Dim, vector<float> x2Dim, 
                    vector<float> x3Dim);
  ...
  void SetAxisNames(TString x1Name, TString x2Name, TString x3Name);
  TString GetAxisName(unsigned int order = 3) const;
  ...
  virtual bool Write(TString FileName, bool Stream = false);
 ...
 private:
  ...
  virtual bool ReadSpecific(MFileResponse& Parser, const TString& Type, const int Version);
