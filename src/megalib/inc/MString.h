 ...
 * MString.h
 *
 * Copyright (C) 2005-2010 by Andreas Zoglauer.
 * All rights reserved.
 *
...
class MString
{
 ...
 public:
  ...
  size_t Last(char c) { return m_String.find_last_of(c); }
  //! Get position of last character or string::npos if none is found
  size_t First(char c) { return m_String.find(c); }

  size_t Index(const MString& S, size_t Start = 0) { return m_String.find(S.m_String, Start); }

  // Conversion:

  operator const char*() const { return m_String.c_str(); }
  operator string() const { return m_String; }
  //operator TString() const { return TString(m_String.c_str()); }
  int ToInt() { return atoi(m_String.c_str()); } 
  double ToDouble() { return atof(m_String.c_str()); }
